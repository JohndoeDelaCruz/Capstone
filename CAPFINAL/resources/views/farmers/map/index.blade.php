<x-app-layout>
    ...
    <script>
        ...
        function initMap() {
            ...
            map = L.map('map').setView([16.5, 120.7], 10);
        ...
        async function loadFilters() {
            try {
                const apiBase = window.location.pathname.includes('/capstone/')
                    ? '/capstone/CAPFINAL/public/api/map'
                    : '/api/map';
        ...
        function updateMap(data) {
            ...
            // Determine path for GeoJSON
            const geojsonPath = window.location.pathname.includes('/capstone/')
                ? '/capstone/CAPFINAL/public/data/benguet.geojson'
                : '/data/benguet.geojson';
        ...
        function updateStats(data) {
            ...
            if (data.metadata) {
                document.getElementById('stat-total').textContent = Number(data.metadata.total).toLocaleString() + ' ' + unit;
                document.getElementById('stat-avg').textContent = Number(data.metadata.average).toLocaleString() + ' ' + unit;
                document.getElementById('stat-max').textContent = Number(data.metadata.max).toLocaleString() + ' ' + unit;
                document.getElementById('stat-min').textContent = Number(data.metadata.min).toLocaleString() + ' ' + unit;
            } else {
